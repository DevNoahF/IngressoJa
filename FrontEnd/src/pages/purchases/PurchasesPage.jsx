...
import { getPurchasedTickets } from "../../utils/tickets";
...
export default function PurchasesPage() {
  ...
  useEffect(() => {
    const stored = getPurchasedTickets();
    ...
    // If there are no tickets stored, show the mock ticket. If stored exists but mock not present, prepend it.
    if (!stored || stored.length === 0) {
      setTickets([mockTicket]);
    } else {
      const hasMock = stored.some(t => String(t.code) === mockTicket.code);
      setTickets(hasMock ? stored : [mockTicket, ...stored]);
    }
  }, []);
              ...
              {tickets.map((t) => (
                <li key={t.id} className="purchase-item">
                  <div className="purchase-left">
                    <img src={t.bannerImage || "https://via.placeholder.com/120x80"} alt={t.eventName} />
                  </div>
                  ...
                  <div className="purchase-right">
                    <div className={`purchase-status ${t.status ? t.status.toLowerCase() : ''}`}>{t.status}</div>
                  </div>
                </li>
              ))}
            </ul>
